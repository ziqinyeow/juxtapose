...
from juxtapose.mmdeploy.codebase.mmdet import get_post_processing_params
from juxtapose.mmdeploy.core import FUNCTION_REWRITER, mark
from juxtapose.mmdeploy.mmcv.ops import multiclass_nms
from juxtapose.mmdeploy.utils import Backend


@FUNCTION_REWRITER.register_rewriter(
    func_name="mmdet.models.dense_heads.rtmdet_head." "RTMDetHead.predict_by_feat"
)
def rtmdet_head__predict_by_feat(
    self,
...
def rtmdet_head__predict_by_feat__ncnn(
    ...
    from juxtapose.mmdeploy.utils.config_utils import is_dynamic_shape
    ...
    flatten_bbox_preds = torch.cat(flatten_bbox_preds, dim=1)
    assert (
        flatten_priors.shape[-1] == 4
    ), f"rtmdet needs (B, N, 4) priors, got\
        (B, N, {flatten_priors.shape[-1]})"

    tl_x = (flatten_priors[:, :, 0:1] - flatten_bbox_preds[:, :, 0:1]) / img_width
    tl_y = (flatten_priors[:, :, 1:2] - flatten_bbox_preds[:, :, 1:2]) / img_height
    br_x = (flatten_priors[:, :, 0:1] + flatten_bbox_preds[:, :, 2:3]) / img_width
    br_y = (flatten_priors[:, :, 1:2] + flatten_bbox_preds[:, :, 3:4]) / img_height
