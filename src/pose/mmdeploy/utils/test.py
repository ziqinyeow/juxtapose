...
class SwitchBackendWrapper:
    ...
    def recover(self):
        ...
        assert (
            self.init is not None and self.forward is not None
...
def get_model_outputs(model: nn.Module, func_name: str, model_inputs: dict) -> Any:
...
def get_onnx_model(
    ...
    patched_model = patch_model(wrapped_model, cfg=deploy_cfg, backend=backend.value)
    ...
    with RewriterContext(
        cfg=deploy_cfg, backend=backend.value, opset=11
    ), torch.no_grad():
        torch.onnx.export(
            model,
            (model_inputs, {}),
...
def get_backend_outputs(
    ...
    if backend == Backend.TENSORRT:
        ...
        input_info = {name: value.shape for name, value in flatten_model_inputs.items()}
        deploy_cfg["backend_config"]["model_inputs"] = [dict(opt_shapes=input_info)]
...
def get_rewrite_outputs(
    wrapped_model: nn.Module,
    model_inputs: Dict[str, Union[Tuple, List, torch.Tensor]],
    deploy_cfg: Config,
    run_with_backend: bool = True,
) -> Tuple[Any, bool]:
    ...
    if run_with_backend:
        backend_outputs = get_backend_outputs(ir_file_path, model_inputs, deploy_cfg)
...
def get_random_name(
    length: int = 10, seed: Optional[Union[int, float, str, bytes, bytearray]] = None
) -> str:
