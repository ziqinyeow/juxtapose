...
class BaseTask(metaclass=ABCMeta):
    ...
    def __init__(
        ...
        if not DefaultScope.check_instance_created(self.experiment_name):
            self.scope = DefaultScope.get_instance(
                self.experiment_name, scope_name=self.model_cfg.get("default_scope")
            )
        else:
    ...
    def build_dataset(
        self, dataset_cfg: Union[str, Config], is_sort_dataset: bool = False, **kwargs
    ) -> Dataset:
        ...
        if is_sort_dataset:
            if is_can_sort_dataset(dataset):
                ...
                logger.info(
                    "Sorting the dataset by 'height' and 'width' " "is not possible."
                )
        return dataset

    @staticmethod
    def build_dataloader(
        dataloader: Union[DataLoader, Dict], seed: Optional[int] = None
    ) -> DataLoader:
    ...
    @staticmethod
    def get_tensor_from_input(input_data: Dict[str, Any], **kwargs) -> torch.Tensor:
    ...
    def from_mmrazor(self) -> bool:
        ...
        if not isinstance(from_mmrazor, bool):
            raise TypeError(
                "`from_mmrazor` attribute must be boolean type! "
