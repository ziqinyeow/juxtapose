...
@FUNCTION_REWRITER.register_rewriter(
    func_name="models.yolox_pose_head." "YOLOXPoseHead.predict"
)
def predict(self, x: Tuple[Tensor], batch_data_samples=None, rescale: bool = True):
...
@FUNCTION_REWRITER.register_rewriter(
    func_name="models.yolox_pose_head." "YOLOXPoseHead.predict_by_feat"
)
def yolox_pose_head__predict_by_feat(
    ...
    vis_preds = torch.cat(
        [
            vis_pred.permute(0, 2, 3, 1).reshape(num_imgs, -1, self.num_keypoints, 1)
    ...
    if backend == Backend.TENSORRT:
        ...
        bboxes = torch.cat(
            [bboxes, bboxes.new_zeros((bboxes.shape[0], 1, bboxes.shape[2]))], dim=1
        )
        scores = torch.cat([scores, scores.new_zeros((scores.shape[0], 1, 1))], dim=1)
        pred_kpts = torch.cat(
            [
                pred_kpts,
                pred_kpts.new_zeros(
