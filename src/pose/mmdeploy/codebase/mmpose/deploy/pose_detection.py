...
def _get_dataset_metainfo(model_cfg: mmengine.Config):
    ...
    for dataloader_name in ["test_dataloader", "val_dataloader", "train_dataloader"]:
        ...
        if hasattr(dataset_mmpose, "_load_metainfo") and isinstance(
            ...
            meta = dataset_mmpose._load_metainfo(dataset_cfg.get("metainfo", None))
            if meta is not None:
                return meta
...
class PoseDetection(BaseTask):
        ...
        **kwargs,
        ...
        if input_shape is not None and hasattr(cfg, "codec"):
            ...
            if tuple(input_shape) != tuple(input_size):
                ...
                logger.warning(
                    f"Input shape from deploy config is not "
            ...
            for i in range(len(bboxes)):
                for bbox in bboxes[i]:
                    ...
                    data = {
                        ...
                        and cfg.model.bbox_head.type == "YOLOXPoseHead"
                        else bbox[None],
                        "img_path": img_path[0],
                    }
    ...
    def visualize(
        ...
        **kwargs,
        ...
        dataset_meta = dataset_meta_from_config(self.model_cfg, dataset_mode="test")
