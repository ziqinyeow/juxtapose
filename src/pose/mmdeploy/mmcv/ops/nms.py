...
class TRTBatchedNMSop(torch.autograd.Function):
    ...
    def forward(
        ...
        ret = (
            torch.rand(batch_size, out_boxes, 5).to(scores.device),
            torch.randint(0, num_classes, (batch_size, out_boxes)).to(scores.device),
        )
        if return_index:
            ret = ret + (
                torch.randint(0, out_boxes, (batch_size, out_boxes)).to(scores.device),
            )
        return ret
...
def _select_nms_index(
    ...
    if output_index and pre_inds is not None:
        ...
        pre_inds = pre_inds.where(
            (batch_inds == batch_template.unsqueeze(1)), pre_inds.new_zeros(1)
        )
...
@FUNCTION_REWRITER.register_rewriter(func_name="mmdeploy.mmcv.ops.nms._multiclass_nms")
def multiclass_nms__default(
    boxes: Tensor,
    scores: Tensor,
    max_output_boxes_per_class: int = 1000,
    iou_threshold: float = 0.5,
...
@mark("multiclass_nms", inputs=["boxes", "scores"], outputs=["dets", "labels", "index"])
def multiclass_nms(
    boxes: Tensor,
    scores: Tensor,
    max_output_boxes_per_class: int = 1000,
...
@FUNCTION_REWRITER.register_rewriter(
    func_name="mmdeploy.mmcv.ops.nms._multiclass_nms", backend=Backend.COREML.value
)
def multiclass_nms__coreml(
    boxes: Tensor,
    scores: Tensor,
    max_output_boxes_per_class: int = 1000,
    iou_threshold: float = 0.5,
    ...
    try:
        ...
        raise Exception("Can not use coreml_nms. Please build torchscript custom ops.")
