...
# visible in mmcv.
@SYMBOLIC_REWRITER.register_symbolic("mmcv.ops.roi_align.__self__", backend="default")
def roi_align_default(
    g,
    input: Tensor,
    rois: Tensor,
    ...
    if backend == Backend.PPLNN or backend == Backend.TENSORRT:
        ...
        rois = select(
            ...
            rois,
            1,
            g.op("Constant", value_t=torch.tensor([1, 2, 3, 4], dtype=torch.long)),
        )
        ...
        if opset_version < 16:
            ...
            if aligned is True:
                rois = add(
                    ...
                    g.op(
                        "Constant",
                        value_t=torch.tensor([-0.5 / spatial_scale], dtype=torch.float),
                    ),
                )
            return g.op(
                "RoiAlign",
